...
class DatabaseManager:
    ...
    def update_alarm_status(self, record_id, alarm_status):
        """Update alarm status for an audio record"""
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        ...
        try:
            c.execute('''
                ...
                SET alarm_status = ?
                WHERE id = ?
            ''', (alarm_status, record_id))
            
            conn.commit()
            logger.info(f"Alarm status updated for record {record_id}: {alarm_status}")
            return True
        except Exception as e:
    ...
    def get_user_alerts(self, user_id, limit=10):
        ...
        c.execute('''
            SELECT id, alert_type, alert_message, urgency_level, is_read, created_at
            FROM user_alerts 
            WHERE user_id = ?
    ...
    def get_user_audio_records(self, user_id, limit=20):
        ...
        c.execute('''
            SELECT id, filename, original_sample_rate, duration, transcribed_text,
        ...
        return [{
            ...
            'duration': record[3],
            'transcribed_text': record[4],
            'urgency_type': record[5],
            'patient_status': record[6],
            'alarm_status': record[7],
            'confidence_score': record[8],
            'created_at': record[9]
    ...
    def get_audio_record_by_id(self, record_id):
        """Get specific audio record by ID"""
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        
        c.execute('''
            SELECT id, user_id, filename, original_sample_rate, file_size, duration,
                   transcribed_text, urgency_type, patient_status, alarm_status, 
        ...
        if record:
            return {
                ...
                'user_id': record[1],
                'filename': record[2],
                'sample_rate': record[3],
                'file_size': record[4],
                'duration': record[5],
                'transcribed_text': record[6],
                'urgency_type': record[7],
                'patient_status': record[8],
                'alarm_status': record[9],
                'confidence_score': record[10],
    ...
    def delete_user_data(self, user_id):
        ...
        try:
            # Delete user alerts
            c.execute('DELETE FROM user_alerts WHERE user_id = ?', (user_id,))
            
            # Delete audio records
            c.execute('DELETE FROM audio_records WHERE user_id = ?', (user_id,))
            
            # Delete analysis stats
            c.execute('DELETE FROM analysis_stats WHERE user_id = ?', (user_id,))
...
def test_database():
    ...
    # Test user creation
    success, message = db.create_user("testuser", "test@example.com", "testpass", "Test User")
    ...
    if user:
        ...
        print(f"Audio record saved with ID: {record_id}")
        
        # Test alarm status update
        if record_id:
            success = db.update_alarm_status(record_id, "Notified to Dr")
            print(f"Alarm status update: {success}")
        ...
        # Test getting audio records
        records = db.get_user_audio_records(user['id'])
        print(f"Audio records: {len(records)} found")
